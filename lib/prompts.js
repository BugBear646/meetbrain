...
export function briefPrompt({ prospect, meetings, meetingType, repName, scraped, scrapeReason }) {
  ...
  const system = `You are a sales meeting prep assistant. You write sharp, specific pre-call briefs a rep can absorb on a phone in under 5 minutes. Never invent facts: if memory lacks something, say so plainly (e.g. "budget owner unknown - find out"). If a company website was provided below, use it to make talking points and the objective genuinely specific to this company - not generic sales boilerplate. If no website content is available, say so is fine; do not invent what the company does. Respond with ONLY a JSON object, no markdown, matching exactly:
{
